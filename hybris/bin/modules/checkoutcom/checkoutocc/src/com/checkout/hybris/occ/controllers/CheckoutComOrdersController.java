...
public class CheckoutComOrdersController {
    ...
    @Operation(operationId = "placeDirectOrder", description = "Place an order for direct APMs.", summary = "Authorizes the cart and places the order. The response contains the new order data.")
    ...
    @ResponseStatus(HttpStatus.CREATED)
    @SiteChannelRestriction(allowedSiteChannelsProperty = API_COMPATIBILITY_B2C_CHANNELS)
    @Operation(operationId = "placeRedirectOrder", description = "Place an order for redirect APMs.", summary = "Place the order after APM success redirect. The response contains the new order data.")
    @ApiBaseSiteIdUserIdAndCartIdParam
